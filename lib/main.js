(function() {
  ...
  this["new"] = function(name) {
    ...
    if (this.timer_by_name[name] != null) {
      throw new Error("timer named " + (rpr(name)) + " already in use");
    }
    R = {
      '~isa': 'TIMER/timer',
      'name': name,
  ...
  this._fetch_timer = function(name) {
    var _ref;
    return (_ref = this.timer_by_name[name]) != null ? _ref : this["new"](name);
  };
  ...
  this.report = function() {
    var R, average_time, average_time_n, bar, color, delta_times, dt, dt_max, dt_max_n, dt_min, dt_n, idx, is_max, is_min, name, number_length, pen, prefix, time, timer, total_time, total_time_txt, _i, _len, _ref, _ref1;
    R = [];
    pen = function() {
      var P;
      P = 1 <= arguments.length ? __slice.call(arguments, 0) : [];
      return R.push(TRM.pen.apply(TRM, P));
    };
    pen();
    ...
    for (name in _ref1) {
      ...
      pen(TRM.grey('average:    '), TRM.orange(this._format_time(timer, average_time, number_length)), bar);
  ...
  this.sync_instrumentalize = function(name, method) {
    var TIMER, timed_method, timer;
    timer = this["new"](name);
    TIMER = this;
    timed_method = function() {
      var P, R, idx;
      P = 1 <= arguments.length ? __slice.call(arguments, 0) : [];
      TIMER.start(timer);
      idx = timer['mru-idx'];
      R = method.apply(null, P);
  ...
  this.async_instrumentalize = function(name, method) {
    var TIMER, timed_method, timer;
    timer = this["new"](name);
    TIMER = this;
    timed_method = function() {
      var P, handler, idx, _i;
